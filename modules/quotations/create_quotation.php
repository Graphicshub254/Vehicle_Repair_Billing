...
// Only procurement officers and directors can create quotations
if (!isProcurementOfficer() && !isDirector()) {
    setErrorMessage('You do not have permission to create quotations');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    for ($i = 1; $i <= $item_count; $i++) {
        if (isset($_POST["part_number_$i"])) {
            $items[] = [
                ...
                'price_excluding_vat' => floatval($_POST["price_excluding_vat_$i"] ?? 0)
            ];
        }
    }
    ...
    if ($job_id <= 0 || $supplier_id <= 0) {
        ...
        try {
            ...
            $quotation_number = generateQuotationNumber($pdo);
            
            // Create quotation
            $stmt = $pdo->prepare("
                INSERT INTO quotations (quotation_number, job_id, supplier_id, quotation_date, status, prepared_by, notes)
                VALUES (?, ?, ?, ?, 'pending_approval', ?, ?)
            ");
            $stmt->execute([$quotation_number, $job_id, $supplier_id, $quotation_date, $_SESSION['user_id'], $notes]);
            ...
            foreach ($items as $item) {
                if ($item['quantity'] > 0 && $item['price_excluding_vat'] > 0) {
                    ...
                    error_log("Debug: Saving item - price_excluding_vat: $price_excluding_vat, vat_amount: $vat_amount, total_amount: $total_amount"); // DEBUG LOG
                    ...
                    ");
                    $stmt->execute([
                        $quotation_id, $item_no++, $item['part_number'], $item['description'],
            ...
            // Log activity
            logActivity($pdo, 'Created quotation', 'quotations', $quotation_id, "Quotation: $quotation_number");
            
            $pdo->commit();
            
            setSuccessMessage("Quotation $quotation_number created successfully and submitted for approval");
            redirect(APP_URL . '/modules/quotations/view_quotation.php?id=' . $quotation_id);
            
        } catch (Exception $e) {
            $pdo->rollBack();
            setErrorMessage('Failed to create quotation: ' . $e->getMessage());
        }
    }
}
...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    <h1><i class="fas fa-file-invoice"></i> Create Quotation</h1>
...
<form method="POST" id="quotationForm">
    ...
    <div class="card">
        <div class="card-footer">
            <a href="<?php echo APP_URL; ?>/modules/quotations/quotations.php" class="btn btn-secondary">Cancel</a>
...
function addItem() {
    ...
    row.innerHTML = `
        <td>${itemCounter}</td>
        <td><input type="text" name="part_number_${itemCounter}" class="form-control form-control-sm" placeholder="e.g., 12345" required></td>
        <td><input type="text" name="description_${itemCounter}" class="form-control form-control-sm" placeholder="Part description"></td>
        <td><input type="number" name="quantity_${itemCounter}" class="form-control form-control-sm" value="1" min="1" onchange="calculateItemTotal(${itemCounter})" required></td>
        <td><input type="number" name="list_price_${itemCounter}" class="form-control form-control-sm" step="0.01" min="0" onchange="calculateItemTotal(${itemCounter})" required></td>
        <td><input type="number" name="discount_price_${itemCounter}" class="form-control form-control-sm" step="0.01" min="0" onchange="calculateItemTotal(${itemCounter})" required></td>
        <td><input type="number" name="price_excluding_vat_${itemCounter}" class="form-control form-control-sm item-price-excluding-vat" step="0.01" min="0" onchange="calculateItemTotal(${itemCounter})" required></td>
